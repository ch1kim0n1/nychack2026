...
export default function ReportPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-canvas">
      ...
      <main className="flex-1 max-w-[840px] mx-auto w-full px-5 sm:px-8 py-8">
        ...
        <article className="bg-surface border border-[var(--cl-border)] rounded shadow-1 print-full-width">
          {/* Report header */}
          <div className="border-b-2 border-navy-800 px-5 sm:px-8 py-6">
          ...
          {/* Business context */}
          {input && (
            <div className="px-5 sm:px-8 py-5 border-b border-[var(--cl-border-subtle)] bg-sunken">
          ...
          {/* Summary */}
          <div className="px-5 sm:px-8 py-5 border-b border-[var(--cl-border-subtle)]">
          ...
          <div className="px-5 sm:px-8 py-5 border-t border-[var(--cl-border-subtle)] bg-sunken">
              ...
              {result.findings.filter(f => f.risk_level === 'high').map((f, i) => (
                <li key={i} className="flex items-start gap-2 text-body">
                  ...
                  <div>
                    ...
                    <span className="font-mono text-citation text-navy-600 block mt-0.5 break-all">
                      Source: {f.source_url}
                    </span>
                  </div>
                </li>
          ...
          {/* Disclaimer footer */}
          <div className="px-5 sm:px-8 py-4 border-t-2 border-navy-800 bg-navy-50">
...
function ReportFinding({ finding, index }: { finding: RiskFinding; index: number }) {
  return (
    <div className="border border-[var(--cl-border)] rounded overflow-hidden">
      {/* Finding header */}
      <div className="flex flex-wrap items-center gap-x-3 gap-y-2 px-4 py-3 border-b border-[var(--cl-border-subtle)] bg-sunken">
        ...
        <h3 className="text-h3 text-[var(--cl-text)] ml-1 w-full sm:w-auto">{finding.affected_area}</h3>
      </div>
      ...
      <div className="px-4 py-4 space-y-3">
        ...
        <div className="font-mono text-citation">
          <a
            ...
            className="inline-flex items-start gap-1 text-navy-600 hover:underline break-all"
          >
            {finding.source_url.replace(/^https?:\/\//, '')}
            <ExternalLink size={10} strokeWidth={1.5} className="mt-1 shrink-0" />
          </a>
        </div>
      </div>
    </div>
